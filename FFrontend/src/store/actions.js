  ...
  export const fetchProduct = (currentPage = 1,result = "",minPrice = 0,maxPrice = 300,category,ratings = 0) => {  
    return async (dispatch) => {
      ...
      let link = `http://localhost:4000/api/v1/products?&page=${currentPage}&keyword=${result}&ratings[gte]=${ratings}`;
      
      if (category){
         link = `http://localhost:4000/api/v1/products?&page=${currentPage}&keyword=${result}&category=${category}`;
      }
